...
#define PROTOCOL_RX_QUEUE_MAX_SIZE 10
...
#elif defined(PROTOCOL_USE_UART)
	UART_set_receive_callback(PROTOCOL_UART_ID, PROTOCOL_onUARTCallback);
#endif
}
...
static bool PROTOCOL_parse(uint8_t* data, size_t data_len, PROTOCOL_t* proto, bool *cleanUp, uint32_t *cutIdx)
{
	...
	uint8_t dataL = data[startByteIdx + 2];
	uint16_t expectedChecksum = PROTOCOL_calCheckSum(&data[startByteIdx + 3], dataL);
	uint16_t checksum = ((uint16_t)data[startByteIdx + 3 + dataL] << 8) | data[startByteIdx + 4 + dataL];
	if(expectedChecksum != checksum)
	{
		return false;
	}
	if(data[startByteIdx + 5 + dataL] != STOP_BYTE)
	{
		return false;
	}
	if(data_len < 6 + dataL)
	...
	proto->protocol_id = data[startByteIdx + 1];
	memcpy(proto->data, &data[startByteIdx + 3], dataL);
...
static uint16_t PROTOCOL_calCheckSum(uint8_t* data, uint8_t data_len)
{
	return CRC_calculate(0, data, data_len);
}
